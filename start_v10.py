...
class RadiatorApp:
    ...
    def load_foreign_spec(self):
        """
        Универсальная загрузка спецификаций других производителей
        Поддерживает Excel, CSV и базовый парсинг PDF
        """
        try:
            # Выбор файла с новыми типами
            ...
            if file_path.lower().endswith('.pdf'):
                df = self._parse_pdf_file(file_path)
            ...
            if df.empty:
                messagebox.showwarning("Ошибка", "Не удалось загрузить данные из файла")
                return

            # Обработка данных
            result = self._process_imported_data(df)
    ...
    def _parse_excel_file(self, file_path):
        ...
        try:
            ...
            preview = pd.read_excel(file_path, engine=engine, header=None, nrows=10)
            
            # Автопоиск строки с заголовками
            ...
            print(f"Excel parsing error: {traceback.format_exc()}")
            return pd.DataFrame()    

    def _find_header_row(self, df_preview):
        """
        Находит строку с заголовками в таблице
        Возвращает номер строки или None если не нашел
        """
        # Ключевые слова для поиска
        keywords = ['артикул', 'наименование', 'кол-во', 'количество', 
                    'art', 'article', 'name', 'qty', 'count']
        
        for row_idx in range(min(5, len(df_preview))):
            # Проверяем все ячейки в строке
            for col_idx in range(len(df_preview.columns)):
                cell_value = str(df_preview.iloc[row_idx, col_idx]).lower()
                if any(keyword in cell_value for keyword in keywords):
                    return row_idx
                    
        # Если не нашли - предлагаем выбрать вручную
        return self._manual_header_selection(df_preview) 

    def _manual_header_selection(self, df_preview):
        """Окно ручного выбора строки заголовков"""
        window = tk.Toplevel(self.root)
        window.title("Выберите строку с заголовками")
        window.geometry("800x400")
        
        # Создаем Treeview для предпросмотра
        tree = ttk.Treeview(window)
        tree["columns"] = list(range(len(df_preview.columns)))
        ...
        for row_idx in range(len(df_preview)):
            tree.insert("", "end", values=list(df_preview.iloc[row_idx]), tags=(f"row{row_idx}",))
        ...
        def on_select():
            ...
            item = tree.focus()
            selected_row = int(tree.item(item, "tags")[0].replace("row", ""))
            window.destroy()
        
        # Кнопка подтверждения
        btn_confirm = ttk.Button(window, text="Выбрать эту строку", command=on_select)
    ...
    def _standardize_columns(self, df):
        """
        Стандартизирует названия столбцов в DataFrame
        Возвращает DataFrame с унифицированными именами колонок
        """
        column_mapping = {}
        ...
        for col in df.columns:
            ...
            if any(x in col_lower for x in ['артикул', 'art', 'код', 'article']):
                column_mapping[col] = 'Артикул'
            elif any(x in col_lower for x in ['наименование', 'name', 'описание']):
                column_mapping[col] = 'Наименование'
            elif any(x in col_lower for x in ['кол-во', 'количество', 'qty', 'count']):
        ...
        required_columns = ['Артикул', 'Наименование', 'Количество']
    ...
    def _parse_csv_file(self, file_path):
        """Парсинг CSV с автоматическим определением разделителя"""
        try:
            ...
            for encoding in encodings:
                try:
                    ...
                    with open(file_path, 'r', encoding=encoding) as f:
                        preview = pd.read_csv(f, sep=None, engine='python', nrows=10)
                    
                    # Если удалось прочитать - продолжаем
    ...
    def _parse_pdf_file(self, file_path):
        """Парсинг PDF файлов с извлечением табличных данных"""
        try:
            import pdfplumber
            import re
            ...
            with pdfplumber.open(file_path) as pdf:
                for page in pdf.pages:
                    ...
                    if tables:
                        ...
                        # Если нет таблиц - извлекаем текст и парсим
                        text = page.extract_text()
                        if text:
                            data_from_text = self._parse_pdf_text(text)
                            if not data_from_text.empty:
                                all_data.append(data_from_text)
            
            # Объединяем все страницы
            if all_data:
                final_df = pd.concat(all_data, ignore_index=True)
                return self._standardize_columns(final_df)
            return pd.DataFrame()
            
        except ImportError:
            messagebox.showerror(
                "Ошибка зависимости", 
                "Для работы с PDF установите pdfplumber:\npip install pdfplumber"
            )
            return pd.DataFrame()
        except Exception as e:
            print(f"PDF parsing error: {traceback.format_exc()}")
            return pd.DataFrame()

    def _parse_pdf_text(self, text):
        """Парсит текстовые данные из PDF когда нет таблиц"""
        try:
            # Шаблоны для поиска данных в тексте
            patterns = [
                # Формат: "Артикул Наименование Количество"
                r"(?P<art>\w+)\s+(?P<name>.+?)\s+(?P<qty>\d+)",
                # Формат: "Наименование: XYZ, Количество: 123"
                r"Наименование:\s*(?P<name>.+?)\s*,\s*Количество:\s*(?P<qty>\d+)",
            ]
            ...
            for line in lines:
                ...
                for pattern in patterns:
                    ...
                    if match:
                        data.append({
                            'Артикул': match.group('art') if 'art' in match.groupdict() else '',
                            'Наименование': match.group('name'),
                            'Количество': match.group('qty')
                        })
                        break
            ...
            print(f"Text parsing error: {traceback.format_exc()}")
            return pd.DataFrame()        
        
    def _process_imported_data(self, df):
        """Основная обработка импортированных данных"""
        result = {
            'total': 0,
            'found': 0,
            'not_found': [],
        ...
        for _, row in df.iterrows():
            original_art = str(row.get('Артикул', '')).strip()
            name = str(row.get('Наименование', '')).strip()
            qty = self.parse_quantity(row.get('Количество', 0))
            
            if not original_art or qty <= 0:
                continue
                
            result['total'] += qty
            
            # 1. Попытка найти по артикулу
            meteor_art, reason = self._find_meteor_analog(original_art, name)
            
            if meteor_art:
                self._add_to_specification(meteor_art, qty)
                result['found'] += qty
            else:
                result['not_found'].append((original_art, name, qty))
                
            result['correspondence'].append({
    ...
    def _find_meteor_analog(self, art, name):
        """Поиск аналога в продукции Meteor"""
        # 1. Проверка в базе прямых соответствий
        direct_match = self._check_direct_matches(art)
        if direct_match:
            return direct_match, "Прямое соответствие"
        
        # 2. Парсинг параметров из названия
        params = self._parse_radiator_params(name)
        if params and all(params.values()):
            # 3. Поиск по параметрам
            param_match = self._find_by_parameters(**params)
            if param_match:
                return param_match, "Подобран по параметрам"
        
        # 4. Попытка найти по ключевым словам
        keyword_match = self._find_by_keywords(name)
        if keyword_match:
            return keyword_match, "Подобран по ключевым словам"
        
        return None, "Аналог не найден"

    def _parse_radiator_params(self, name):
        """Извлекает параметры из названия радиатора"""
        # Унифицируем название для парсинга
        name = name.replace('×', 'x').replace('*', 'x').lower()
        
        # Различные форматы названий
        patterns = [
            # Тип 11/500/1000
            r"(?:тип|type)\s*(\d+)[/\\x\s]*(\d+)[/\\x\s]*(\d+)",
            # Kermi FKO 11 500 1000
            r"(?:\w+\s+)?\w+\s+(\d+)\s+(\d+)\s+(\d+)",
            # Purmo Compact 22 x 600 x 1000
            r"\d+\s*x\s*\d+\s*x\s*\d+"
        ]
    ...
    def _find_by_keywords(self, name):
        """
        Ищет аналог радиатора Meteor по ключевым словам в названии.
        Возвращает артикул Meteor или None, если совпадений нет.
        """
        if not name:
            return None

        name = str(name).lower()  # Приводим к нижнему регистру для удобства сравнения

        # Пример простого поиска по ключевым словам (можно расширить)
        keywords_map = {
            "тип 10": "10",
            "тип 11": "11",
            "тип 20": "20",
            ...
            "тип 30": "30",
            "тип 33": "33",
            "радиатор": "10",  # По умолчанию, если просто "радиатор"
        }

        # Ищем совпадения ключевых слов
        for keyword, rad_type in keywords_map.items():
            if keyword in name:
                # Возвращаем первый подходящий артикул из базы (пример)
                for sheet_name, data in self.sheets.items():
                    if f"тип {rad_type}" in sheet_name.lower():
                        return data.iloc[0]["Артикул"]  # Берём первый попавшийся артикул

        return None  # Если ничего не найдено 
    
    def _check_direct_matches(self, art):
        """
        Проверяет наличие прямого соответствия артикула в базе данных.
        Возвращает артикул Meteor, если найдено соответствие, иначе None.
        """
        art = str(art).strip().upper()  # Нормализуем артикул
        
        # Проверяем во всех листах матрицы
        for sheet_name, data in self.sheets.items():
            if art in data['Артикул'].astype(str).str.strip().str.upper().values:
                return art  # Возвращаем оригинальный артикул, если он есть в базе
        
        # Проверяем в кронштейнах
        if not self.brackets_df.empty and art in self.brackets_df['Артикул'].astype(str).str.strip().str.upper().values:
            return art
        
        return None  # Если соответствие не найдено

    def _show_import_result(self, result_data):
        """Показывает результаты импорта и позволяет ручное сопоставление"""
        window = tk.Toplevel(self.root)
        window.title("Результаты загрузки")
        ...
        # Статистика
        stats_text = {
            f"Загружено позиций: {len(result_data['correspondence'])}",
            f"Найдено аналогов: {result_data['found']} из {result_data['total']}"
        }

        ttk.Label(top_frame, text=stats_text).pack(side='left')
        ...
        ttk.Button(btn_frame, text="Сохранить", 
                command=lambda: self.save_import_results(result_data)).pack(side='left')
        ttk.Button(btn_frame, text="Закрыть", 
                command=window.destroy).pack(side='left', padx=5)
        
        # Таблица результатов
        columns = ["original_art", "original_name", "meteor_art", "reason", "qty"]
        tree = ttk.Treeview(bottom_frame, columns=columns, show='headings')
        
        # Настройка столбцов
        tree.heading("original_art", text="Артикул оригинала")
        tree.heading("original_name", text="Наименование")
        tree.heading("meteor_art", text="Аналог Meteor")
        tree.heading("reason", text="Статус")
        ...
        # Заполнение данными
        for item in result_data['correspondence']:
            values = [item[col] for col in columns]
            tree.insert("", "end", values=values, tags=(item['reason'],))
        
        # Цветовое выделение строк
        tree.tag_configure("Прямое соответствие", background='#e6ffe6')
        tree.tag_configure("Подобран по параметрам", background='#fffae6')
        tree.tag_configure("Аналог не найден", background='#ffe6e6')
        
        # Прокрутка
        scroll_y = ttk.Scrollbar(bottom_frame, orient='vertical', command=tree.yview)
        scroll_x = ttk.Scrollbar(bottom_frame, orient='horizontal', command=tree.xview)
        ...
        # Контекстное меню для ручного сопоставления
        self._setup_context_menu(tree, result_data)
        
        # Запускаем окно
        window.mainloop()

    def _setup_context_menu(self, tree, result_data):
        """Настраивает контекстное меню для ручного сопоставления"""
        menu = tk.Menu(tree, tearoff=0)
        
        menu.add_command(
            label="Указать аналог вручную",
            command=lambda: self._manual_analog_selection(tree, result_data)
        )
        
        menu.add_command(
            label="Исключить из спецификации",
            command=lambda: self._exclude_item(tree, result_data)
        )
    ...
    def _manual_analog_selection(self, tree, result_data):
        """Окно ручного выбора аналога"""
        selected_item = tree.focus()
        if not selected_item:
            return
            
        item_data = tree.item(selected_item)
        original_art = item_data['values'][0]
        
        # Создаем окно выбора
        select_window = tk.Toplevel(self.root)
        select_window.title(f"Выбор аналога для {original_art}")
        ...
        # Таблица с аналогами
        analogs_tree = ttk.Treeview(select_window, columns=('art', 'name'), show='headings')
        analogs_tree.heading('art', text='Артикул')
        analogs_tree.heading('name', text='Наименование')
        analogs_tree.column('art', width=150)
        analogs_tree.column('name', width=500)
        
        # Заполняем всеми радиаторами Meteor
        for sheet_name, sheet_data in self.sheets.items():
            for _, row in sheet_data.iterrows():
                analogs_tree.insert("", "end", 
                                values=(row['Артикул'], row['Наименование']),
                                tags=(sheet_name,))
        
        # Прокрутка
        scroll = ttk.Scrollbar(select_window, orient='vertical', command=analogs_tree.yview)
        analogs_tree.configure(yscrollcommand=scroll.set)
        
        analogs_tree.pack(side='left', fill='both', expand=True)
        scroll.pack(side='right', fill='y')
        
        # Кнопка выбора
        def on_select():
            selected_analog = analogs_tree.item(analogs_tree.focus())['values'][0]
            qty = item_data['values'][4]
            
            # Обновляем данные
            for item in result_data['correspondence']:
                if item['original_art'] == original_art:
                    item['meteor_art'] = selected_analog
                    item['reason'] = "Ручное сопоставление"
                    break
            ...
            tree.item(selected_item, values=(
                ...
                qty
            ), tags=("Ручное сопоставление",))
            
            # Добавляем в спецификацию
            self._add_to_specification(selected_analog, qty)
            select_window.destroy()
        
        ttk.Button(select_window, text="Выбрать", command=on_select).pack(pady=10) 

    def _add_to_specification(self, meteor_art, qty):
        """Добавляет позицию в спецификацию"""
        # Находим лист с этим артикулом
        for sheet_name, sheet_data in self.sheets.items():
            if meteor_art in sheet_data['Артикул'].values:
                # Добавляем или обновляем количество
                key = (sheet_name, meteor_art)
                current_qty = self.entry_values.get(key, 0)
                self.entry_values[key] = str(int(current_qty) + qty) if current_qty else str(qty)
                break
    ...
    def _save_import_results(self, result_data):
        """Сохраняет результаты импорта в файл"""
        ...
        try:
            ...
            df = pd.DataFrame(result_data['correspondence'])
