...
class RadiatorApp:
    def __init__(self, root):
        ...
        x = (screen_width - 400) // 2  # 800 - примерная начальная ширина окна
    ...
    def log_error(self, message):
        ...
        print(message)  # В консоль для дебага
        
        # Если окно логов существует, выводим туда
    ...
    def set_window_icon(self):
        """Устанавливает иконку окна с обработкой ошибок"""
        try:
    ...
    def load_foreign_spec(self):
        """Загружает спецификацию от других производителей с подробным логированием"""
        # Создаем окно для логов
        self.log_window = tk.Toplevel(self.root)
        self.log_window.title("Лог загрузки")
        self.log_window.geometry("800x500")
        
        # Текстовое поле с прокруткой
        log_text = tk.Text(self.log_window, wrap=tk.WORD, state='disabled')
        scrollbar = ttk.Scrollbar(self.log_window, command=log_text.yview)
        log_text.configure(yscrollcommand=scrollbar.set)
        
        # Кнопка закрытия
        close_btn = ttk.Button(self.log_window, text="Закрыть", 
        ...
        def log_message(message):
            ...
            self.log_window.update()  # Обновляем окно
        
        log_message("=== Начало загрузки ===")
        ...
        try:
            ...
            log_message(f"Выбран файл: {file_path}")
            ...
            try:
                ...
                log_message(f"Успешно прочитано строк: {len(df)}")
            except Exception as e:
                log_message(f"Ошибка чтения файла: {str(e)}")
                return
            
            # Автопоиск столбцов
            name_col, qty_col = None, None
            for idx, row in df.head(3).iterrows():  # Проверяем первые 3 строки
                for col in df.columns:
                    cell_value = str(row[col]).lower()
                    
                    # Ищем столбец с наименованием
                    if not name_col and any(x in cell_value for x in ["наименование", "описание"]):
                        name_col = col
                        log_message(f"Найден столбец наименований: {col}")
                    
                    # Ищем столбец с количеством
                    if not qty_col and any(x in cell_value for x in ["кол-во", "количество", "qty"]):
                        qty_col = col
                        log_message(f"Найден столбец количества: {col}")
            ...
            if not name_col or not qty_col:
                name_col, qty_col = 0, 1
                log_message("Столбцы не определены, используются первые два")
            ...
            for idx, row in df.iterrows():
                try:
                    ...
                    # Здесь должна быть логика сопоставления с радиаторами Meteor
                    # ...
                    
                    total_loaded += 1
                    log_message(f"Обработано: {name} x{qty}")
                    
                except Exception as e:
                    log_message(f"Ошибка в строке {idx+1}: {str(e)}")
            
            log_message(f"=== Итого загружено: {total_loaded} позиций ===")
    ...
    def extract_radiator_params(self, name):
        """Извлекает параметры радиатора из названия"""
        result = {
            'recognized': False,
            'connection': None,
            'type': None,
        ...
        try:
            ...
            patterns = [
                # Пример: "Радиатор VK 22-500-600"
                r'(?:радиатор|radiator).*?([ckv])\s*(\d+)[-\s](\d+)[-\s](\d+)',
                # Пример: "ЛК 11-504"
                r'[лl][кk]\s*(\d+)[-\s](\d+)'
            ]
            ...
            for pattern in patterns:
                ...
                if match:
                    if len(match.groups()) == 4:  # Первый паттерн
                        connection = 'VK-правое' if match.group(1).upper() in ['V', 'C'] else 'K-боковое'
                        ...
                        height = 500  # Стандартная высота для Лидея
            ...
            print(f"Ошибка извлечения параметров: {str(e)}")
        
        return result
    def select_meteor_analog_for_import(self):
        """Специальная версия для обработки импорта"""
        if not hasattr(self, '_current_import_item'):
            return
        
        item = self._current_import_item
        self.select_meteor_analog(None, None)  # Вызываем диалог выбора
        # Дополнительная логика для обработки импорта        

    def transfer_to_matrix(self, tree, correspondence_df):
        """Переносит выбранные значения из таблицы соответствия в матрицу"""
        selected_items = tree.selection()
        if not selected_items:
            messagebox.showwarning("Ошибка", "Выберите строки для переноса")
            return
        ...
        for item in selected_items:
            ...
            if not meteor_art:
                messagebox.showwarning("Ошибка", f"Для строки '{original_name}' не выбран аналог METEOR")
                continue
            
            # Ищем лист, где находится этот артикул
            sheet_name = None
            for sheet in self.sheets:
                if meteor_art in self.sheets[sheet]['Артикул'].values:
                    sheet_name = sheet
                    break
            
            if not sheet_name:
                messagebox.showerror("Ошибка", f"Артикул {meteor_art} не найден в матрице")
                continue
            
            # Добавляем количество в матрицу
            current_value = self.entry_values.get((sheet_name, meteor_art), "")
            if current_value:
                new_value = f"{current_value}+{qty}"
            else:
                new_value = str(qty)
            
            self.entry_values[(sheet_name, meteor_art)] = new_value
        
        # Обновляем матрицу
        self.refresh_matrix()
        self.global_highlight()
        messagebox.showinfo("Успех", "Выбранные позиции перенесены в матрицу")        
    ...
    def select_meteor_analog(self, tree, correspondence_df):
        """Позволяет пользователю выбрать аналог из матрицы"""
        selected_items = tree.selection()
        if not selected_items:
            messagebox.showwarning("Ошибка", "Выберите строку для сопоставления")
            return
        
        # Берем только первую выбранную строку
        item = selected_items[0]
        values = tree.item(item, "values")
        original_name = values[0]
        qty = values[1]
        
        # Создаем диалоговое окно для выбора аналога
        dialog = tk.Toplevel(self._correspondence_window)
        dialog.title("Выбор аналога METEOR")
        ...
        # Информация о выбранном радиаторе
        ttk.Label(main_frame, 
                text=f"Выбран радиатор: {original_name}\nКоличество: {qty}", 
        ...
        ttk.Label(conn_frame, text="Тип подключения:").pack(side="left")
        connection_var = tk.StringVar(value="VK-правое")
        ttk.Combobox(
            conn_frame,
            textvariable=connection_var,
            values=["VK-правое", "VK-левое", "K-боковое"],
            state="readonly"
        ...
        ttk.Label(type_frame, text="Тип радиатора:").pack(side="left")
        type_var = tk.StringVar(value="10")
        ttk.Combobox(
            type_frame,
            textvariable=type_var,
            values=["10", "11", "20", "21", "22", "30", "33"],
            state="readonly"
        ).pack(side="left", padx=5)
        
        # Матрица для выбора размеров
        ...
        for col, h in enumerate(heights):
            ttk.Label(matrix_frame, text=f"{h} мм", width=10).grid(row=0, column=col+1)
        
        # Строки (длины) и кнопки выбора
        lengths = range(400, 2100, 100)
        for row, l in enumerate(lengths, start=1):
            ttk.Label(matrix_frame, text=f"{l} мм").grid(row=row, column=0, padx=5)
            
            for col, h in enumerate(heights, start=1):
                btn = ttk.Button(
                    matrix_frame, 
                    text="Выбрать", 
                    width=8,
                    command=lambda l=l, h=h: self._set_meteor_analog(
                        tree, item, connection_var.get(), type_var.get(), l, h, dialog)
                )
    ...
    def show_correspondence_table(self, correspondence_df):
        ...
        # Кнопка выбора аналога
        ttk.Button(
            button_frame,
            text="Выбрать аналог",
            command=lambda: self.select_meteor_analog(tree, correspondence_df)
        ).pack(side="left", padx=5)
        
        # Кнопка переноса в матрицу
        ttk.Button(
            button_frame,
            text="Перенести в матрицу",
            command=lambda: self.transfer_to_matrix(tree, correspondence_df)
        ).pack(side="left", padx=5)
    ...
    def parse_quantity(self, value):
        """
        Преобразует введенное значение в целое число (количество радиаторов).
        Обрабатывает: числа, строки с числами, пустые значения и ошибки.
        
        Параметры:
            value - любое значение (число, строка, None и т.д.)
        
        Возвращает:
            int - количество (0 при ошибке)
        """
        # Если значение пустое или NaN (пропущенное значение в Excel)
        ...
        # Если строка пустая после обработки
        if not str_value:
            return 0
        
        try:
            # Пробуем преобразовать в целое число
            return int(float(str_value))  # Сначала в float, чтобы обработать "2.0"
        except ValueError as e:
            # Логируем ошибку, если не удалось преобразовать
            error_msg = f"Ошибка преобразования количества: '{str_value}' не является числом"
