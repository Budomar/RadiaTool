...
class RadiatorApp:
    ...
    def ask_user_for_mapping(self, competitor_name, qty):
        """Запрашивает у пользователя соответствие для нераспознанного радиатора и сохраняет его"""
        ...
        # Информация о радиаторе конкурента
        ttk.Label(main_frame, 
                text=f"Радиатор конкурента не распознан:", 
        ...
        ttk.Label(main_frame, 
                text="Выберите аналог METEOR из списка:", 
                font=("Segoe UI", 10, "bold")).pack(anchor="w", pady=(10, 0))
        
        # Фрейм для выбора
        select_frame = ttk.Frame(main_frame)
        select_frame.pack(fill="both", expand=True, pady=10)
        
        # Treeview для выбора радиаторов METEOR
        columns = ["Тип подключения", "Тип радиатора", "Высота", "Длина", "Артикул", "Наименование"]
        tree = ttk.Treeview(select_frame, columns=columns, show="headings", selectmode="browse")
        ...
        # Заполняем данными
        for sheet_name, data in self.sheets.items():
            connection = sheet_name.split()[0]
            rad_type = sheet_name.split()[1]
            
            for _, row in data.iterrows():
                # Извлекаем высоту и длину из наименования
                name_parts = row['Наименование'].split('/')
                if len(name_parts) >= 3:
                    height = name_parts[-2].replace('мм', '').strip()
                    length = name_parts[-1].replace('мм', '').strip().split()[0]
                    
                    tree.insert("", "end", values=[
                        connection, rad_type, height, length, 
                        row['Артикул'], row['Наименование']
                    ])
        
        # Прокрутка
        vsb = ttk.Scrollbar(select_frame, orient="vertical", command=tree.yview)
        hsb = ttk.Scrollbar(select_frame, orient="horizontal", command=tree.xview)
        ...
        def on_select():
            nonlocal selected_meteor
            selected_item = tree.focus()
            if selected_item:
                selected_meteor = tree.item(selected_item, "values")
                ...
                mappings[competitor_name] = {
                    'meteor_art': selected_meteor[4],
                    'meteor_name': selected_meteor[5],
                    'connection': selected_meteor[0],
                    'rad_type': selected_meteor[1],
                    'height': selected_meteor[2],
                    'length': selected_meteor[3]
                }
        ...
        ttk.Button(button_frame, text="Отмена", command=on_cancel).pack(side="left", padx=5)
        
        # Ждем закрытия диалога
        dialog.wait_window()
        
        if selected_meteor:
            # Добавляем в текущую спецификацию
            sheet_name = f"{selected_meteor[0]} {selected_meteor[1]}"
            art = selected_meteor[4]
    ...
    def load_foreign_spec(self):
        ...
        # Загружаем сохраненные соответствия
        mappings_file = self.resource_path("mappings.json")
        mappings = {}
        if os.path.exists(mappings_file):
            try:
        ...
        try:
            ...
            # Определяем столбцы с наименованием и количеством
            name_col = None
            qty_col = None
            ...
            # Если не нашли стандартные названия - анализируем содержимое столбцов
            if name_col is None or qty_col is None:
                # Пытаемся определить столбцы автоматически
                for col in df.columns:
            ...
            for _, row in df.iterrows():
                try:
                    ...
                    # Пропускаем строки, которые явно не радиаторы
                    if not any(x in name.lower() for x in ['радиатор', 'radiator', 'k-profil', 'vk-profil']) and \
                    not re.match(r'^[cv]\s*\d+', name, re.IGNORECASE):
                        continue

                    # 1. Сначала проверяем сохраненные соответствия
                    if name in mappings:
                        # Используем сохраненное соответствие
                        meteor_art = mappings[name]['meteor_art']
                        sheet_name = f"{mappings[name]['connection']} {mappings[name]['rad_type']}"
                        
                        if (sheet_name, meteor_art) in self.entry_values:
                            current_qty = self.parse_quantity(self.entry_values[(sheet_name, meteor_art)])
                            self.entry_values[(sheet_name, meteor_art)] = str(current_qty + qty)
                        else:
                            self.entry_values[(sheet_name, meteor_art)] = str(qty)
                        
                        correspondence_df.loc[len(correspondence_df)] = [
                            name, qty, 
                            mappings[name]['meteor_name'], meteor_art,
                            "Загружен из сохраненных соответствий"
                        ]
                    ...
                    # 2. Пытаемся автоматически определить параметры из названия
                    meteor_match = None
                    
                    # Шаблоны для распознавания:
                    # 1. Панельный стальной радиатор Valve Compact тип VС 22-500-600
                    pattern1 = re.compile(
                        r'(?:радиатор|radiator).*?тип\s*([ckv])\s*(\d+)[-\s](\d+)[-\s](\d+)',
                        re.IGNORECASE
                    )
                    # 2. Радиаторы стальные штампованные "Лидея" ЛК 11-504
                    pattern2 = re.compile(
                        r'(?:радиатор|radiator).*?[лl][кk]\s*(\d+)[-\s](\d+)',
                        re.IGNORECASE
                    )
                    # 3. VC 22-500-600
                    pattern3 = re.compile(
                    ...
                    # 4. ЛК 11-504
                    pattern4 = re.compile(r'[лl][кk]\s*(\d+)[-\s](\d+)', re.IGNORECASE)
                    ...
                    if meteor_match:
                        # Определяем параметры в зависимости от шаблона
                        if meteor_match.re == pattern1 or meteor_match.re == pattern3:
                            # Для шаблонов 1 и 3: тип, серия, высота, длина
                        ...
                        elif meteor_match.re == pattern2 or meteor_match.re == pattern4:
                            # Для шаблонов 2 и 4: тип, длина (высота фиксированная)
                            rad_type = meteor_match.group(1)
                            length = int(meteor_match.group(2))
                            
                            # Для Лидея высота определяется по типу
                        ...
                        # Формируем имя листа для поиска
                        sheet_name = f"{connection} {rad_type}"
                        
                        if sheet_name in self.sheets:
                            # Ищем радиатор с нужными параметрами
                            data = self.sheets[sheet_name]
                            ...
                            if not match.empty:
                                ...
                                correspondence_df.loc[len(correspondence_df)] = [
                                    ...
                                    "Автоматически определен по параметрам"
                                ]
                                
                                total_loaded += 1
                                total_qty += qty
                                similar_loaded.append(name)
                                continue
                    
                    # 3. Если автоматически не удалось определить аналог, спрашиваем пользователя
                    if not self.ask_user_for_mapping(name, qty):
                        not_found.append(f"{name} (не найден аналог Meteor)")
                        correspondence_df.loc[len(correspondence_df)] = [
                            name, qty, "", "", 
                            "Не найден аналог Meteor"
                        ]
                except Exception as e:
